...
function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { setError(err.response?.data?.message || 'Unable to login.') }
    ...
    <main className="relative flex min-h-svh items-center justify-center px-6 py-10" style={{ background: 'var(--bg-primary)' }}>
      ...
      <section className="animate-scale-in relative z-10 w-full max-w-sm">
        <div className="mb-10 text-center">
          ...
          <h1 className="text-2xl font-extrabold" style={{ color: 'var(--text-primary)' }}>Log in to RhythmicTunes</h1>
        </div>
        ...
        <div className="glass-strong rounded-xl p-7">
          ...
          <div className="mt-6 text-center">
            ...
            <Link to="/register" className="text-sm font-bold underline" style={{ color: 'var(--accent)' }}>Sign up</Link>
          </div>
        </div>
      </section>
    </main>
